...
@cache()
async def buscarInformacoes(cnpj: str) -> tuple:
        ...
        try:
            async with session.get(url) as response:
                if response.status == 200:
                    ...
                    decreto = cnae_codigo in CNAES_VALIDOS
                    simples_valor = bool(simples)

                    return razao_social, cnae_codigo, decreto, uf, simples_valor
                else:
                    print(f"API respondeu com status {response.status}")
...
async def processarCnpjs(lista_cnpjs: list[str]) -> dict:
